...
describe('TemplateFormComponent', () => {
  ...
  function getFormError(fieldName) {
    const elem = fixture.debugElement.query(By.css(`.form-${fieldName}-error`));
  ...
  it('has "username" form control', () => {
    expect(getFormFieldNames()).toContain(
      'username',
      'Missing NgModel or FormControl'
    );
  });

  it('should validate username is required', fakeAsync(() => {
    // get control
    ...
    // set value
    control && control.setValue('admin');
    // expect valid

    tick();
    expect(control && control.valid).toBeTruthy(
      'Username valid when not empty'
    );
  }));
  ...
  const invalidPasswords = [
    ...
    },
    {
      password: 'Password',
      message:
        'length is ok, both upper and lowercase chars present, but no number',
    },
    {
      password: 'pa55word',
  ...
  it('should validate password with pattern - at least 8 letters, numbers and uppercase', () => {
    ...
    for (const { message, password } of invalidPasswords) {
      ...
      expect(control && control.valid).toBeFalsy(
        `Password should be invalid: ${message}`
      );
    }
    ...
    for (const { message, password } of validPasswords) {
      control && control.setValue(password);
      expect(control && control.valid).toBeTruthy(
        `Password should be valid: ${message}`
      );
    }
  });
  ...
  it('should validate passwords match', () => {
    ...
    // expect invalid
    expect(control_match && control_match.valid).toBeFalsy(
      'Match should be invalid'
    );
    ...
    // expect valid
    expect(control_match && control_match.valid).toBeTruthy(
  ...
  it('has .form-username-error when username is invalid', () => {
    ...
    fixture.detectChanges();
    expect(getFormError('username')).toBeFalsy(
    ...
    fixture.detectChanges();
    expect(getFormError('username-required')).toBeTruthy(
  ...
  it('has .form-password_match-error when password_match is invalid', () => {
    ...
    control && control.setErrors(null);
    fixture.detectChanges();
    expect(getFormError('password_match')).toBeFalsy(
      'Error message should not be present'
    );

    // Make field invalid
    control && control.setErrors({ passwordMismatch: true });
    fixture.detectChanges();
    expect(getFormError('password_match')).toBeTruthy(
      'Error message should be present'
    );
  });
});
...
@Component({
  template: `
    <template-form (save)="submit($event)"></template-form>
  `,
})
...
describe('TemplateFormComponent Inputs Outputs', () => {
